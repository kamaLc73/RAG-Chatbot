...
Les IDs peuvent etre les IDs bruts de forms.json ou les anciens IDs Vespa `form::<id>`.
"""
...
def evaluate_forms(args: argparse.Namespace) -> dict[str, Any]:
    ...
    retriever = FormRetriever(
        ...
        top_k_retrieve=args.target_hits,
        top_k_final=args.hits,
        relevance_threshold=args.threshold,
    )
    if not retriever.available:
        logger.warning("FormRetriever ChromaDB indisponible. Lance index_forms.py avant le benchmark final.")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        try:
            if not retriever.available:
                raise RuntimeError("FormRetriever ChromaDB indisponible")
            hits = retriever.retrieve(query, org=org)
        except Exception as exc:
            error = repr(exc)

        latency = time.perf_counter() - started
        hit_ids = [normalize_form_id(hit.get("form_id", "")) for hit in hits]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluation du retrieval des formulaires ChromaDB")
