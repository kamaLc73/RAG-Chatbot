...
def evaluate_videos(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not retriever.available:
        logger.warning("VideoRetriever ChromaDB indisponible. Lance index_videos.py avant le benchmark final.")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        try:
            if not retriever.available:
                raise RuntimeError("VideoRetriever ChromaDB indisponible")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluation du retrieval des videos ChromaDB")
