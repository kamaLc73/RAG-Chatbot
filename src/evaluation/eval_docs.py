...
Exemples:
  ...
  .\\.venv\\Scripts\\python.exe src\\evaluation\\eval_docs.py --run-name docs_full
...
DEFAULT_CEREBRAS_API_KEY_ENV = os.getenv("RAGAS_CEREBRAS_API_KEY_ENV", "CEREBRAS_API_KEY")
DEFAULT_CEREBRAS_REASONING_EFFORT = os.getenv("RAGAS_CEREBRAS_REASONING_EFFORT", "low").strip().strip('"\'')
DEFAULT_CEREBRAS_REASONING_FORMAT = os.getenv("RAGAS_CEREBRAS_REASONING_FORMAT", "hidden").strip().strip('"\'')
...
def make_starter_dataset(path: Path, per_org: int = 24, overwrite: bool = False) -> None:
...
def query_local_rag(rag: Any, question: str, org: str) -> dict[str, Any]:
    ...
    try:
        try:
            payload = rag.query(question, org=org, include_contexts=True, use_intent_classifier=False)
        except TypeError:
...
def get_api_key_from_env(env_name: str) -> str:
    ...
    if not api_key:
        raise RuntimeError(f"{env_name} est requis pour l'evaluation RAGAS via Cerebras.")
    return api_key
...
def setup_judge(
    ...
    cerebras_requests_per_minute: float,
    cerebras_max_completion_tokens: int,
    cerebras_api_key_env: str,
    ...
    cerebras_api_key = get_api_key_from_env(cerebras_api_key_env)

    requests_per_second = max(cerebras_requests_per_minute, 1.0) / 60.0
    ...
    if "gpt-oss" in cerebras_model.lower():
        if cerebras_reasoning_effort:
            llm_kwargs["reasoning_effort"] = cerebras_reasoning_effort
        extra_body = {}
        if cerebras_reasoning_format:
            extra_body["reasoning_format"] = cerebras_reasoning_format
        if extra_body:
...
def run_ragas(
    ...
    llm_wrapper, emb_wrapper = setup_judge(
        ...
        cerebras_requests_per_minute=cerebras_requests_per_minute,
        cerebras_max_completion_tokens=cerebras_max_completion_tokens,
        cerebras_api_key_env=cerebras_api_key_env,
...
def write_summary_markdown(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Judge model: `{summary.get('judge_llm', '')}`",
        f"- Judge base URL: `{summary.get('judge_base_url', '')}`",
        f"- Judge max completion tokens: `{summary.get('judge_max_completion_tokens', '')}`",
        f"- Judge reasoning effort: `{summary.get('judge_reasoning_effort', '')}`",
        f"- Judge reasoning format: `{summary.get('judge_reasoning_format', '')}`",
        f"- Judge RPM: `{summary.get('judge_rpm', '')}`",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--judge-base-url", default=DEFAULT_CEREBRAS_BASE_URL, help="Endpoint OpenAI-compatible Cerebras")
    parser.add_argument(
        "--judge-rpm",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Variable d'environnement contenant la cle Cerebras",
    )
    parser.add_argument(
        "--judge-reasoning-effort",
        default=DEFAULT_CEREBRAS_REASONING_EFFORT,
        help="Effort de raisonnement pour gpt-oss via Cerebras: low, medium ou high",
    )
    parser.add_argument(
        "--judge-reasoning-format",
        default=DEFAULT_CEREBRAS_REASONING_FORMAT,
        help="Format de raisonnement pour gpt-oss via Cerebras, ex: hidden ou parsed",
    )
...
def main() -> int:
    ...
    if not args.skip_ragas:
        ...
        scores = run_ragas(
            dataset=dataset,
            cerebras_model=args.judge_model,
            cerebras_base_url=args.judge_base_url,
            cerebras_requests_per_minute=args.judge_rpm,
            cerebras_max_completion_tokens=args.judge_max_completion_tokens,
            cerebras_api_key_env=args.judge_api_key_env,
            cerebras_reasoning_effort=args.judge_reasoning_effort,
            cerebras_reasoning_format=args.judge_reasoning_format,
    ...
    summary: dict[str, Any] = {
        ...
        "judge_reasoning_format": args.judge_reasoning_format,
